...
local function LoadNPCs()
    ...
    for i, npc in ipairs(NPCs) do
        ...
        debugLog(string.format("^3[NPCs] Loaded NPC %d: Model=%s, Name=%s, Coords=%s, Heading=%s^7", 
            i, npc.model, npc.name or "Unnamed", 
...
AddEventHandler('npcs:addToConfig', function(npcData)
    ...
    local success = MySQL.insert.await('INSERT INTO npcs (model, coords_x, coords_y, coords_z, heading, anim_dict, anim_name, name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
        npcData.model,
        npcData.coords.x,
        npcData.coords.y,
        npcData.coords.z,
        ...
        npcData.name or nil -- Added name field
    })
    
    if success then
        print(string.format("^2[NPCs] Successfully saved NPC to database: Model=%s, Name=%s, Coords=%s, Heading=%s^7", 
            npcData.model, 
            npcData.name or "Unnamed", 
